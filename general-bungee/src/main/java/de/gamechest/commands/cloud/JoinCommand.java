    ...
    public void execute(CommandSender sender, String[] args) {
        if(!(sender instanceof ProxiedPlayer))
            ...
            if(args.length == 1) {
                ...
                if(con == 0) {
                    pp.sendMessage("§7Verbinde zum Server "+srvid+"...");
//                    pp.sendMessage(Cloud.PREFIX+"§7Verbinde zum Server "+srvid+"...");
                    return;
                }
                if(con == 1) {
                    pp.sendMessage("§cDu befindest dich schon auf diesem Server!");
//                    pp.sendMessage(Cloud.PREFIX+"§cDu befindest dich schon auf diesem Server!");
                    return;
                }
                if(con == 2) {
                    pp.sendMessage("§cDer Server existiert nicht!");
//                    pp.sendMessage(Cloud.PREFIX+"§cDer Server existiert nicht!");
                    return;
                }
                pp.sendMessage("§cError: ("+con+") Konnte keine richtige ID finden.");
//                pp.sendMessage(Cloud.PREFIX+"§cError: ("+con+") Konnte keine richtige ID finden.");
            ...
            if(args.length == 2) {
                ...
                if(args[0].equals("@all")) {
                    ...
                    ProxyServer.getInstance().broadcast("§7Alle Spieler wurden auf §e"+srvid+"§7 verschoben!");
//                    ProxyServer.getInstance().broadcast(Cloud.PREFIX+"§7Alle Spieler wurden auf §e"+srvid+"§7 verschoben!");
                ...
                if(target == null) {
                    pp.sendMessage("§cDer Spieler ist nicht online!");
//                    pp.sendMessage(Cloud.PREFIX+"§cDer Spieler ist nicht online!");
                ...
                if(rankId > Rank.DEVELOPER.getId() &&
                        ...
                        < rankId) {
                    pp.sendMessage("§cDu hast keine Berechtigung diesen Spieler zu verschieben!");
//                    pp.sendMessage(Cloud.PREFIX+"§cDu hast keine Berechtigung diesen Spieler zu verschieben!");
                ...
                if(con == 0) {
                    target.sendMessage("§7Verbinde zum Server "+srvid+"...");
//                    target.sendMessage(Cloud.PREFIX+"§7Verbinde zum Server "+srvid+"...");

                    pp.sendMessage("§7Verschiebe "+target.getName()+" zum Server "+srvid+"...");
//                    pp.sendMessage(Cloud.PREFIX+"§7Verschiebe "+target.getName()+" zum Server "+srvid+"...");
                    return;
                }
                if(con == 1) {
                    pp.sendMessage("§c"+target.getName()+" befindest dich schon auf diesem Server!");
...
//                pp.sendMessage(Cloud.PREFIX+"§cError: ("+con+") Konnte keine richtige ID finden.");
            ...
            pp.sendMessage("§c/join <Server> (Spieler)");
